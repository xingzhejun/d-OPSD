...
def print_prompt_completions_sample(prompts: list[str], completions: list[str], rewards: list[int], step: int, teacher_prompts: list[str] | None = None) -> None:
    """
    ...
    if teacher_prompts is not None:
        table.add_column("Teacher Prompt", style="bright_magenta")

    if teacher_prompts is not None:
        for prompt, completion, reward, teacher_prompt in zip(prompts, completions, rewards, teacher_prompts):
            table.add_row(
                Text(prompt),
                Text(completion),
                f"{reward:.2f}",  # Formatting reward to 2 decimal places
                Text(teacher_prompt)
            )
